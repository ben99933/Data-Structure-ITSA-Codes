...
Request* newRequest(int time, int deadline){
...
int cmpInt(const Object o1, const Object o2){
    return (*(int*)o1 - *(int*)o2);
}

int cmpIntReverse(const Object o1, const Object o2){
    return -(*(int*)o1 - *(int*)o2);
}

int cmpRequest(const Object o1, const Object o2){
    Request* r1 = (Request*)o1;
    Request* r2 = (Request*)o2;
    int result = cmpInt(&r1->deadline, &r2->deadline);
    if(result == 0)return cmpInt(&r1->time, &r2->time);
    return result;
}
...
int main(){ 
    ...
    for(int i = 0;i<n;i++){
        ...
        request[i] = newRequest(time, deadline);
    }
    //為啥qsort有問題QQ
    for(int i = 0;i<n;i++){
        int fix = 0;
        for(int j = 0;j<n-i-1;j++){
            int result = cmpRequest(request[j], request[j+1]);
            if(result > 0){
                Request* temp = request[j];
    ...
    int time = 0;
    for(int i = 0;i<n;i++){
        if(time+request[i]->time <= request[i]->deadline){
            time += request[i]->time;
        }else{
            succ = false;
            break;
